...
#ifdef _7ZIP_LARGE_PAGES
extern
bool g_DismLargePagesMode;
bool g_DismLargePagesMode = false;
#endif
    ...
    bool wildcardMatching)
{
  ...
  if (!pair.Prepare())
  {
    ...
    if (type == NRecursedType::kRecursed)
      ...
      val += "-r0";
    throw DismCArcCmdLineException("Unsupported rename command:", val);
  }
}
    ...
    const CNameOption &nop, LPCWSTR fileName, UInt32 codePage)
{
  ...
  if (!NFind::DoesFileExist_FollowLink(us2fs(fileName)))
    throw DismCArcCmdLineException(kCannotFindListFile, fileName);
  */
  DWORD lastError = 0;
  if (!ReadNamesFromListFile2(us2fs(fileName), names, codePage, lastError))
  {
    if (lastError != 0)
    {
      ...
      throw DismCArcCmdLineException(m, fileName);
    }
    throw DismCArcCmdLineException(kIncorrectListFile, fileName);
  }
  if (renamePairs)
  {
    if ((names.Size() & 1) != 0)
    ...
    bool thereAreSwitchIncludes, UInt32 codePage)
{
  ...
  for (unsigned i = startIndex; i < nonSwitchStrings.Size(); i++)
  {
    ...
    if (s.IsEmpty())
      throw DismCArcCmdLineException(kEmptyFilePath);
  ...
  if (oldIndex != -1)
  {
    throw DismCArcCmdLineException("There is no second file name for rename pair:", nonSwitchStrings[(unsigned)oldIndex]);
  }
}
    ...
    UInt32 codePage)
{
  ...
  if (i != strings.Size())
    throw DismCArcCmdLineException(errorMessage, strings[i]);
}
    ...
    const NUpdateArchive::CActionSet &defaultActionSet)
{
  ...
  if (i != updatePostStrings.Size())
    throw DismCArcCmdLineException(errorMessage, updatePostStrings[i]);
}
  ...
  if (parser[NDismKey::kVolume].ThereIs)
  {
    ...
    FOR_VECTOR (i, sv)
    {
      ...
      if (!ParseComplexSize(sv[i], size) || size == 0)
        throw DismCArcCmdLineException("Incorrect volume size:", sv[i]);
    ...
    CWimCmdLineOptions &options)
{
  ...
  if (!parser.ParseStrings(kSwitchForms, ARRAY_SIZE(kSwitchForms), commandStrings))
    throw DismCArcCmdLineException(parser.ErrorMessage, parser.ErrorLine);
  ...
  if (parser[NDismKey::kLogLevel].ThereIs)
  {
    ...
    else
    {
      ...
      if (!StringToUInt32(s, v))
        throw DismCArcCmdLineException("Unsupported switch postfix -bb", s);
  ...
  if (parser[NDismKey::kLargePages].ThereIs)
  {
    ...
    else if (s != L"-")
    {
      if (!StringToUInt32(s, slp))
        throw DismCArcCmdLineException("Unsupported switch postfix for -slp", s);
    }
  ...
  if (parser[NDismKey::kAffinity].ThereIs)
  {
    ...
    if (!s.IsEmpty())
    {
      ...
      if (a.IsEmpty())
        throw DismCArcCmdLineException("Unsupported switch postfix -stm", s);

      {
        #ifndef _WIN64
        if (v >= ((UInt64)1 << 32))
          throw DismCArcCmdLineException("unsupported value -stm", s);
        #endif
        {
    ...
    bool byteOnlyCodePages, Int32 defaultVal)
{
  ...
  for (unsigned i = 0;; i++)
  {
    if (i == num) // to disable warnings from different compilers
      throw DismCArcCmdLineException("Unsupported charset:", name);
...
void CWimCmdLineParser::Parse2(CWimCmdLineOptions &options)
{
  ...
  if (numNonSwitchStrings < kMinNonSwitchWords)
    throw DismCArcCmdLineException("The command must be specified");
  ...
  if (parser[NDismKey::kHashGenFile].ThereIs)
  {
    ...
    for (unsigned i = 0 ; i < s.Len();)
    {
      ...
      {
        if (c != '=')
          throw DismCArcCmdLineException("Unsupported hash mode switch:", s);
  ...
  */
  ...
  if (fullPathMode)
  {
    ...
    if (!s.IsEmpty())
    {
      if (s == L"2")
        ...
        throw DismCArcCmdLineException("Unsupported -spf:", s);
    }
  }
  ...
  if (parser[NDismKey::kUseSlashMark].ThereIs)
  {
    ...
    if (s.IsEmpty())
      ...
      throw DismCArcCmdLineException("Unsupported -spm:", s);
  }
  ...
  if (thereIsArchiveName)
  {
    if (curCommandIndex >= numNonSwitchStrings)
      throw DismCArcCmdLineException("Cannot find archive name");
    options.ArchiveName = nonSwitchStrings[curCommandIndex++];
    if (options.ArchiveName.IsEmpty())
      throw DismCArcCmdLineException("Archive name cannot by empty");
    #ifdef _WIN32
  ...
  if (isExtractOrList)
  {
    ...
    if (parser[NDismKey::kZoneFile].ThereIs)
    {
      ...
      if (!s.IsEmpty())
      {
          ...
          throw DismCArcCmdLineException("Unsupported -snz:", s);
      }
    }
    ...
    if (!options.Censor.AllAreRelative())
      throw DismCArcCmdLineException("Cannot use absolute pathnames for this command");
  ...
  else if (options.Command.CommandType == NDismCommandType::kDismBenchmark)
  {
    ...
    if (curCommandIndex < numNonSwitchStrings)
    {
      if (!StringToUInt32(nonSwitchStrings[curCommandIndex], options.NumIterations))
        throw DismCArcCmdLineException("Incorrect number of benchmark iterations", nonSwitchStrings[curCommandIndex]);
      curCommandIndex++;
